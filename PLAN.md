...
| `pnpm dev basic`           | basic           | basic    |
| `pnpm dev harness-wrapper` | harness-wrapper | basic    |

- **Backend** runs on port 3001
- **Frontend** runs on port 3000

Backends and frontends are just subdirectories in `server/` and `web/`.
...
`server/harness-wrapper`: Wraps basic server and adds harness integrations for Pi (/agents/pi/_), Claude (/agents/claude/_), and OpenCode (/agents/opencode/\*) agents.

`web/basic`: A web frontend to interact with the server
...
**MCP**
: Connecting to MCP servers via oauth etc . MCP servers are also examples of "stateful" tools. The question is: what should happen when I run connectToMCPServer({}). And how should that be recovered after session restart etc? Can we have an abstraction that MCP is built on top of, rather than being somehow burrowed into our agent core?
